...
TEST(UpdateSetTest, forkedParallelUpdateSetShouldBeSequentialWhenRequestingSequential) {
    const auto updateSet = std::unique_ptr<UpdateSet>(new UpdateSet(UpdateSet::Type::Parallel, 0));
    const auto forkedUpdateSet = std::unique_ptr<UpdateSet>(updateSet->fork(UpdateSet::Type::Sequential, 0));
...
TEST(UpdateSetTest, forkedParallelUpdateSetShouldBeParallelWhenRequestingParallel) {
    const auto updateSet = std::unique_ptr<UpdateSet>(new UpdateSet(UpdateSet::Type::Parallel, 0));
    const auto forkedUpdateSet = std::unique_ptr<UpdateSet>(updateSet->fork(UpdateSet::Type::Parallel, 0));
...
TEST(UpdateSetTest, forkedSequentialUpdateSetShouldBeParallelWhenRequestingParallel) {
    const auto updateSet = std::unique_ptr<UpdateSet>(new UpdateSet(UpdateSet::Type::Sequential, 0));
    const auto forkedUpdateSet = std::unique_ptr<UpdateSet>(updateSet->fork(UpdateSet::Type::Parallel, 0));
    EXPECT_EQ(UpdateSet::Type::Parallel, forkedUpdateSet->type());
}
...
TEST(UpdateSetTest, mergingParallelUpdateSetsIntoSequentialOnesShouldOverrideLocationValues) {
    ...
    const auto seqUpdateSet = std::unique_ptr<UpdateSet>(new UpdateSet(UpdateSet::Type::Sequential, 0));
    seqUpdateSet->add(&location, new Update{.value = make_integer_value(10)});

    const auto parUpdateSet = std::unique_ptr<UpdateSet>(seqUpdateSet->fork(UpdateSet::Type::Parallel, 0));
    parUpdateSet->add(&location, new Update{.value = make_integer_value(1000)});

    parUpdateSet->merge();

    EXPECT_EQ(make_integer_value(1000), seqUpdateSet->lookup(&location)->value);
}
...
TEST(UpdateSetTest, mergingUpdateSetsIntoEmptyUpdateSetsShouldSwapValues) {
    ...
    parUpdateSet->add(&location2, new Update{.value = make_integer_value(2000)});
    parUpdateSet->add(&location3, new Update{.value = make_integer_value(3000)});
...
TEST(UpdateSetTest, lookupShouldPreferUpdatesOfCurrentUpdateSet) {
    ...
    const auto seqUpdateSet1 = std::unique_ptr<UpdateSet>(new UpdateSet(UpdateSet::Type::Sequential, 0));
    seqUpdateSet1->add(&location, new Update{.value = make_integer_value(1)});

    const auto parUpdateSet = std::unique_ptr<UpdateSet>(seqUpdateSet1->fork(UpdateSet::Type::Parallel, 0));

    const auto seqUpdateSet2 = std::unique_ptr<UpdateSet>(parUpdateSet->fork(UpdateSet::Type::Sequential, 0));
    seqUpdateSet2->add(&location, new Update{.value = make_integer_value(2)}); // winner

    EXPECT_EQ(make_integer_value(2), seqUpdateSet2->lookup(&location)->value);
}
...
TEST(UpdateSetTest, lookupShouldConsiderAllSequentialParentUpdateSets) {
    ...
    const auto parUpdateSet1 = std::unique_ptr<UpdateSet>(seqUpdateSet1->fork(UpdateSet::Type::Parallel, 0));

    const auto seqUpdateSet2 = std::unique_ptr<UpdateSet>(parUpdateSet1->fork(UpdateSet::Type::Sequential, 0));
    seqUpdateSet2->add(&location, new Update{.value = make_integer_value(2)}); // winner

    const auto parUpdateSet2 = std::unique_ptr<UpdateSet>(seqUpdateSet2->fork(UpdateSet::Type::Parallel, 0));
    parUpdateSet2->add(&location, new Update{.value = make_integer_value(0)});

    const auto seqUpdateSet3 = std::unique_ptr<UpdateSet>(parUpdateSet2->fork(UpdateSet::Type::Sequential, 0));

    EXPECT_EQ(make_integer_value(2), seqUpdateSet3->lookup(&location)->value);
}
...
TEST(UpdateSetManagerTest, forkAndMerge) {
    ...
    manager->fork(UpdateSet::Type::Parallel, 0);
    ...
    {
        manager->fork(UpdateSet::Type::Sequential, 0);
