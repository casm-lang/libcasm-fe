...
TEST( AstValueLiteral, toStringOfBoolean )
{
    // 'true'
    {
        const auto filename = std::make_shared< std::string >( TEST_CASE );
        const auto location = SourceLocation( { filename, 10, 10 }, { filename, 10, 10 + 4 } );
        const auto value = libstdhl::Memory::get< libcasm_ir::BooleanConstant >( true );
        const auto literal = Ast::make< ValueLiteral >( location, value );
        EXPECT_EQ( literal->toString(), "true" );
    }
    // 'false'
    {
        const auto filename = std::make_shared< std::string >( TEST_CASE );
        const auto location = SourceLocation( { filename, 10, 10 }, { filename, 10, 10 + 5 } );
...
{
    {
        const auto radix = libstdhl::Type::Radix::BINARY;
        {
            const auto filename = std::make_shared< std::string >( TEST_CASE );
            const auto location =
                SourceLocation( { filename, 10, 10 - 2 }, { filename, 10, 10 + 1 } );
        ...
        }
        {
            const auto filename = std::make_shared< std::string >( TEST_CASE );
            const auto location =
                SourceLocation( { filename, 10, 10 - 2 }, { filename, 10, 10 + 13 } );
            const auto value =
        ...
        }
        {
            const auto filename = std::make_shared< std::string >( TEST_CASE );
            const auto location =
                SourceLocation( { filename, 10, 10 - 2 }, { filename, 10, 10 + 5 } );
            const auto value =
...
TEST( AstValueLiteral, toStringOfInteger )
{
    ...
    {
        const auto filename = std::make_shared< std::string >( TEST_CASE );
        const auto location = SourceLocation( { filename, 10, 10 }, { filename, 10, 10 + 1 } );
    ...
    {
        const auto filename = std::make_shared< std::string >( TEST_CASE );
        const auto location = SourceLocation( { filename, 10, 10 - 1 }, { filename, 10, 10 + 1 } );
        const auto value = libstdhl::Memory::get< libcasm_ir::IntegerConstant >( "-1", radix );
        const auto literal = Ast::make< ValueLiteral >( location, value );
        EXPECT_EQ( literal->toString(), "-1" );
    }
    ...
    {
        const auto filename = std::make_shared< std::string >( TEST_CASE );
        const auto location = SourceLocation( { filename, 10, 10 - 1 }, { filename, 10, 10 + 7 } );
        const auto value =
