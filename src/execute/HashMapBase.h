...
namespace details
{
    namespace HashingStrategy
    {
        ...
        {
            static /*constexpr*/ std::size_t initialSize(std::size_t size) noexcept
            {
                return nextPowerOfTwo(size);
            }

            static /*constexpr*/ std::size_t nextSize(std::size_t size) noexcept
            {
                assert(isPowerOfTwo(size));
                return size * 2;
            }

            static constexpr std::size_t hash(std::size_t hashCode,
        ...
        private:
             ...
             */
            static /*constexpr*/ std::size_t nextPowerOfTwo(std::size_t n) noexcept
            {
                n -= 1;
                n |= (n >> 1);
                n |= (n >> 2);
...
private:
    ...
    void growIfNecessary()
    {
        if (m_buckets == nullptr) {
            resize(HashingStrategy::initialSize(m_capacity)); // initial resize
