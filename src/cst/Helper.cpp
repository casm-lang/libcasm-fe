...
Initially::Initially( const Token::Ptr& equalToken, const Expression::Ptr& expression )
: Helper( Node::ID::INITIALLY, Token::unresolved(), Token::unresolved() )
, m_expression( expression )
, m_equalToken( equalToken )
{
}

const Expression::Ptr& Initially::expression( void ) const
{
    return m_expression;
}

const Token::Ptr& Initially::equalToken( void ) const
{
    return m_equalToken;
}
...
{
    auto duplicate =
        std::make_shared< Initially >( equalToken(), expression()->duplicate< Expression >() );
