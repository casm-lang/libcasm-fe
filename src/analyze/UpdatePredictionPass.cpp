...
#include "UpdatePredictionPass.h"
...
char UpdatePredictionPass::id = 0;

static libpass::PassRegistration<UpdatePredictionPass> PASS
( "Update Prediction Pass"
, "tries to predict the size of the update set"
, "update"
, 0
);

UpdatePredictionPass::UpdatePredictionPass() :
    m_evaluatedRules()
{

}

bool UpdatePredictionPass::run(libpass::PassResult& pr)
{
...
void UpdatePredictionPass::visit_rule_post(RuleNode* node)
{
    node->updates = node->child_->updates;
}

void UpdatePredictionPass::visit_statements_post(AstListNode* stmts)
{
...
void UpdatePredictionPass::visit_ifthenelse_post(IfThenElseNode* node, bool)
{
    if (node->else_) {
...
void UpdatePredictionPass::visit_case(CaseNode* node, const bool, const std::vector<bool>&)
{
...
void UpdatePredictionPass::visit_call_post(CallNode* node)
{
...
void UpdatePredictionPass::visit_let_post(LetNode* node)
{
    node->updates = node->stmt->updates;
}

void UpdatePredictionPass::visit_iterate_post(UnaryNode* node)
{
...
void UpdatePredictionPass::visit_forall_post(ForallNode* node)
{
...
void UpdatePredictionPass::visit_seqblock_post(UnaryNode* node)
{
    node->updates = node->child_->updates;
}

void UpdatePredictionPass::visit_parblock_post(UnaryNode* node)
{
    node->updates = node->child_->updates;
}

bool UpdatePredictionPass::visit_update(UpdateNode* update, bool, bool)
{
    update->updates = 1;
    return true;
}

bool UpdatePredictionPass::visit_update_dumps(UpdateNode* update, bool, bool)
{
    update->updates = 1;
    return true;
}

void UpdatePredictionPass::visit_push(PushNode* node, bool, bool)
{
    node->updates = 1;
}

void UpdatePredictionPass::visit_pop(PopNode* node)
{
...
void UpdatePredictionPass::evaluateRule(RuleNode* rule)
{
    m_evaluatedRules.emplace(rule);
    AstWalker<libcasm_fe::UpdatePredictionPass, bool> walker{*this};
    walker.suppress_calls = true;
    walker.walk_rule(rule);
}

bool UpdatePredictionPass::isRuleEvaluated(RuleNode* rule) const
{
