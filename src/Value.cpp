...
value_t::value_t( INTEGER_T integer )
: type( TypeType::INTEGER )
{
    value.integer = integer;
}

value_t::value_t( uint64_t bitvalue )
: type( TypeType::BIT )
{
    value.integer = (INTEGER_T)bitvalue;
}

value_t::value_t( FLOATING_T float_ )
...
value_t::value_t( const Type& t, List* list )
: type( t.t )
{
    value.list = list;
}

value_t::value_t( const enum_value_t* enum_val )
: type( TypeType::ENUM )
{
    value.enum_val = enum_val;
}

value_t::value_t( const rational_t* rat )
: type( TypeType::RATIONAL )
{
    value.rat = rat;
}

value_t::value_t( symbol_t* sym )
...
bool value_t::operator==( const value_t& other ) const
{
    ...
    switch( type )
    {
        ...
        case TypeType::FLOATING:
            return value.float_ == other.value.float_;
        case TypeType::BOOLEAN:
            return value.boolean == other.value.boolean;
        case TypeType::ENUM:
            return value.enum_val == other.value.enum_val;
        case TypeType::RATIONAL:
            return *value.rat == *other.value.rat;
        case TypeType::STRING:
            return *value.string == *other.value.string;
        case TypeType::TUPLE:
        case TypeType::TUPLE_OR_LIST:
        case TypeType::LIST:
            return *value.list == *other.value.list;
        case TypeType::NUMBER_RANGE:
            return *value.numberRange == *other.value.numberRange;
        case TypeType::RULEREF:
            return value.rule == other.value.rule;
        default:
            FAILURE();
    }
}
...
const std::string value_t::to_str( bool symbolic ) const
{
    switch( type )
    {
        ...
        default:
            throw RuntimeException( "Unsupported type in value_t.to_str() " );
    }
}
...
NumberRange::const_iterator& NumberRange::const_iterator::operator=(
...
bool NumberRange::const_iterator::operator==( const const_iterator& rhs ) const
    noexcept
{
...
NumberRange::const_iterator::const_reference NumberRange::const_iterator::
    ...
    else if( ptr->is_skip() )
    {
        ...
        else
        {
            if( skip->bottom->tail )
            {
                ...
                size_t i;
                size_t additional_skip
                    = skip->skip - skip->bottom->values.size();
...
List::const_iterator::const_iterator( const self_type& other )
: bottom( other.bottom )
, head( other.head )
, pos( other.pos )
{
...
HeadList::HeadList( List* l, const value_t& val )
: List( ListType::HEAD )
, right( l )
, current_head( val )
{
...
TailList::TailList( TailList* l, const value_t& val )
: List( ListType::TAIL )
, right( l )
, current_tail( val )
{
...
SkipList::SkipList( size_t skip, BottomList* btm )
: List( ListType::SKIP )
, skip( skip )
, bottom( btm )
{
